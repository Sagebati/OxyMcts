    ...
    EV: Evaluator<A>,
{
    ...
    pub fn execute(&mut self, playout_args: PP::Args) {
        let node = TP::tree_policy(&mut self.tree, self.root_state.clone());
        let final_state = PP::playout(node.1, playout_args);
        let eval = EV::evaluate_leaf(&final_state, &self.root_state.player_turn());
        BP::backprop(&mut self.tree, node.0, eval);
    }
    ...
    pub fn best_move(&self) -> <EV::State as GameTrait>::Move {
        ...
        self.tree
            ...
            .state
            .last()
            .expect( "The historic of the children of the root is empty" )
            .clone()
    }
}
